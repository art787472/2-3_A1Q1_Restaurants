...
router.get('/search', (req, res) => {
  ...
  return Restaurants.find({ [searchFilter]: { '$regex': keyword, '$options': 'i' }, userId})
    .lean()
    .sort({ [sortBy]: order })
    .then(restaurants => {
      if (restaurants.length === 0) {
        return Restaurants.find({ userId }).lean().then(restaurants => {
    ...
    .then(restaurants => {
      res.render('index', { pageTitle: indexTitle, styleSheetLink: '/stylesheets/index.css', restaurants: restaurants.results, isNotFind: restaurants.isNotFind, searchKeyword: input, sortIcon })
    })
    .catch(err => {
      return res.render('error', { errorMessage: err })
    })
